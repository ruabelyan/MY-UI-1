import { ShadowThemeConfig } from './types';

const shadowDefaultValues: ShadowThemeConfig = {
  xsm: '0px 1px 2px rgba(16, 24, 40, 0.05)',
  sm: '0px 1px 2px -1px rgba(16, 24, 40, 0.1), 0px 1px 3px rgba(16, 24, 40, 0.1)',
  md: '0px 2px 4px -2px rgba(16, 24, 40, 0.1), 0px 4px 6px -1px rgba(16, 24, 40, 0.1)',
  lg: '0px 4px 6px -4px rgba(16, 24, 40, 0.1), 0px 10px 15px -3px rgba(16, 24, 40, 0.1)',
  xlg: '0px 8px 10px -6px rgba(16, 24, 40, 0.1), 0px 20px 25px -5px rgba(16, 24, 40, 0.1)',
};

export default shadowDefaultValues;
